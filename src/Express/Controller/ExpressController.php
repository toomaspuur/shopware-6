...
class ExpressController extends StorefrontController
{
    ...
    public function callback(Request $request, RequestDataBag $inputData, SalesChannelContext $salesChannelContext): Response
    {
        ...
        */
        ...
        if ($isValid === true) {
            $data = $inputData->all();
            $this->logger->info('cookies:' . \print_r($request->cookies->all(), true));

            $shipping = $data['shipping'] ?? null;
            if (\is_array($shipping) && isset($shipping['shippingAddress'])) {
                try {
                    $countryValid = $this->expressService->switchShippingContext($shipping['shippingAddress'], $salesChannelContext);
    ...
    public function confirmAction(Request $request, RequestDataBag $inputData, SalesChannelContext $salesChannelContext): Response
    {
        ...
        if ($isValid === true) {
            try {
                ...
                if ($contextToken !== $tempData[PlatformRequest::HEADER_CONTEXT_TOKEN]) {
                    throw new IvyException('invalid sw-context-token ' . $contextToken);
                }
    ...
    public function finishAction(Request $request, RequestDataBag $inputData, SalesChannelContext $salesChannelContext)
    {
        try {
            ...
            $this->expressService->updateIvyExpressOrder($ivyPaymentSession, $swOrder->getOrderNumber(), $ivyOrderId, $salesChannelContext);
